...
@given(
    start_end_dates=date_interval(
        min_value=datetime(1950, 1, 1), max_value=datetime(2100, 12, 31)
    )()
)
def test_creating_and_reading_filenames_consistency(start_end_dates):
    ...
    for variable, frequency in cart_prod(variables, frequencies):
        ...
        output_filename = get_output_filename(
            variable, frequency, start_date, end_date, domain_name
        )
        ...
        extracted_start, extracted_end = from_file_path_to_time_range(
            file_path
        )
